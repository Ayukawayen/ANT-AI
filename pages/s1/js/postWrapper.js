function refreshPostChain(keys) {
    let parentNode = document.querySelector('#內文 .posts');
	
	let nodes = document.querySelectorAll('#內文 .posts>li');
...
function scrollPostToBottom() {
	document.querySelector('#內文 .content').lastElementChild.scrollIntoView(false);
}
...
function refreshPostNav(key) {
    let node = document.querySelector(`#內文 .posts [key="${key}"]`);
