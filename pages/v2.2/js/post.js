...
window.addEventListener('load', onLoad_Post);
...
async function onPostClick(node, ev) {
	...
	if(op == 'cancel') {
		node.querySelector('textarea.frontend').value = post.contents['frontend']||'';
		node.querySelector('textarea.llm').value = post.contents['llm']||'';
		
		node.setAttribute('mode', 'text');
	}
	if(op == 'save') {
		post.contents['frontend'] = node.querySelector('textarea.frontend').value;
		post.contents['llm'] = node.querySelector('textarea.llm').value;
		
		onPostChange();
		
		updatePostContentNode(node, post.contents);
		
		node.setAttribute('mode', 'text');
	}
	
	if(op == 'toLlm') {
		let text = node.querySelector('textarea.frontend').value;
		
		let translated = await sendTranslateRequest('frontend', 'llm', key, text);

		node.querySelector('textarea.llm').value = translated;
	}
	if(op == 'toFrontend') {
		let text = node.querySelector('textarea.llm').value;
		
		let translated = await sendTranslateRequest('llm', 'frontend', key, text);

		node.querySelector('textarea.frontend').value = translated;
	}
}
...
function onPostChange() {
	...
	localStorage.setItem(chatId + '.post', JSON.stringify(data));
	onChatChange();
}

function onLoad_Post() {
	let data = localStorage.getItem(chatId + '.post');
	if(!data) return;
...
function refreshPostNavNode(node, key) {
	...
	node.querySelector('.control .text >span').textContent = index + ' / ' + size;
}

function updatePostContentNode(node, contents) {
	['frontend', 'llm'].forEach((type)=>{
		node.querySelector(`.edit .${type}`).value = contents[type];

		let textNode = node.querySelector(`.text .${type}`);
		
		textNode.innerHTML = '';
		
		let text = mapPostText(contents[type]);
...
function addPost(arg) {
	...
	posts[key].contents[ posts[key].role == 'assistant' ? 'llm' : 'frontend' ] = arg.content||'';
