...
function onUsertextSubmitClick() {
	...
	if(text) {
		...
		_onAdded(userKey, posts[userKey].parent);
	}
...
function onPostClick(node, ev) {
	...
	if(op == 'left' || op == 'right') {
		...
		updateFirestore_Rotate(parent);
	}
	
	if(op == 'delete') {
		let delParent = parent;
		removePost(key);
		
		let k = getDescendantPostKey(delParent);
		updateCurrentPostKey(k);
		
		updateFirestore_Delete(delParent);
	}
	if(op == 'add') {
	...
	if(op == 'regen') {
		if(post.role == 'assistant') {
			let hadChildren = posts[key].childs && posts[key].childs.length > 0;
			let regenParent = parent;
			
			sendRequest({parent: regenParent, replace: key, _hadChildren: hadChildren});
		}
	}
	...
	if(op == 'save') {
		...
		if(!isChatExist) {
			initFirestore_NewChat();
		} else {
			updateFirestore_EditSave(key);
		}
	}
}

function _onAdded(newKey, parentKey) {
	if(!isChatExist) {
		initFirestore_NewChat();
	} else {
		updateFirestore_Added(newKey, parentKey);
	}
}
