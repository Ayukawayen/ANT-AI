...
console.log(resp);
		if(resp.error) {
			...
			if(options.replace) {
				// Regen：在刪除舊節點前記錄是否有子代
				let hadChildren = (options._hadChildren === true);
				let regenParent = options.parent;
				let oldKey = options.replace;
				...
				// Firestore：一次性寫入 Regen 結果
				updateFirestore_Regen(oldKey, newKey, regenParent, hadChildren);
			} else {
				// 一般新增 assistant post（Added）
				let newKey = addPost({
				...
				_onAdded(newKey, options.parent);
			}
		}
		setNotAutoSend(false);
		scrollPostToBottom();
	} catch (error) {
		console.error(error);
...
function buildMessages(options) {
	...
	let msgs = [];
	let regex = /<MESSAGE role="([^"]*)">\s*(.*?)\s*<\/MESSAGE>/gis;
	let match;
	let countToken = 0;
