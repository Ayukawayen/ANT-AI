...
function onPostClick(node, ev) {
	...
	if(op == 'save') {
		...
		updatePostContentNode(node, content);
...
function updatePostNode(node, key) {
	...
	node.querySelector('textarea').value = post.content||'';
	...
	updatePostContentNode(node, post.content);
}
...
function updatePostContentNode(node, content) {
