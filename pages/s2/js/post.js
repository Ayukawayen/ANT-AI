...
function onPostClick(node, ev) {
	...
	if(op == 'save') {
		...
		updatePostContentNode(node, content, reasoning);
...
function updatePostNode(node, key) {
	...
	updatePostContentNode(node, post.content, post.reasoning);
}
...
function updatePostContentNode(node, content, reasoning) {
	
	node.querySelector('.edit textarea').value = `<think>${reasoning||''}</think>\n${content}`;
